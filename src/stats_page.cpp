...
Component CreateStatsPage(int *selected_page, Character *character, Closure exit_closure) {
    ...
    auto renderer = Renderer(container, [back_button, save_button,quit_button, character,
                                 experience_bar, progress_bar,encumberance_bar, equipment_items,patent_items,
                                 inventory_items,
                                 countdown_timer] {
                                 ...
                                 Element character_stats = vbox({
                                                               ...
                                                               hbox({
                                                                   text("NDAs: ") | bold,
                                                                   text(std::to_string(character->ndas))
                                                               }),
                                                               hbox({
                                                                   text("Luck: ") | bold,
                                                                   text(std::to_string(character->luck))
                                                               }),
                                                               hbox({
                                                                   text("Synergy: ") | bold,
                                                               ...
                                                               hbox({
                                                                   text("Technical Debt: ") | bold,
                                                                   text(std::to_string(character->techdebt))
                                                               }),
                                                               hbox({
                                                                   text("Bus Factor: ") | bold,
                                 ...
                                 Element experience_gauge = window(text("Funding") | bold | center, vbox({
                                                                       gauge(experience_bar->GetProgress()) | color(
                                                                           Color::Yellow),
                                                                       text("Series " + std::to_string(
                                                                                1 + int(
                                                                                    experience_bar->GetProgress() * 10))
                                                                            +
                                                                            " (" + std::to_string(
                                                                                int(experience_bar->GetProgress() *
                                                                                    100)) + "%)") | center,
                                                                   }));

                                 // Column 1: Patents

                                 Element patents = vbox({
                                                       text("Patents") | bold | center,
                                                       separator(),
                                 ...
                                 Element inventory = vbox({
                                                         ...
                                                         vbox({
                                                             gauge(encumberance_bar->GetProgress()) | color(Color::Red),
                                 ...
                                 Element equipment = vbox({
                                                     ...
                                                     }) | border | size(HEIGHT, LESS_THAN, 22);

                                 // Bottom progress bar
                                 Element progress_gauge = window(
                                                              text("Progress") | bold | center,
                                                              vbox({
                                                                  text("Axing the Monkey " + std::to_string(
                                                                           int(progress_bar->GetProgress() * 100)) +
                                                                       "%"),
                                                                  gauge(progress_bar->GetProgress()) | color(
                                                                      Color::Blue)
